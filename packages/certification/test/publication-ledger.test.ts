...
describe("publication ledger", () => {
  ...
  it("records an exact initial-release rollback as latest/next removal plus deprecation", () => {
    ...
    expect(operations.map(({ packageName, action, tag, after }) => ({ packageName, action, tag, after })).slice(0, 3)).toEqual([
      { packageName: "@pixel-point/aval-svelte", action: "rollback-tag", tag: "latest", after: null },
      { packageName: "@pixel-point/aval-svelte", action: "rollback-tag", tag: "next", after: null },
      { packageName: "@pixel-point/aval-svelte", action: "deprecate", tag: "deprecated", after: "Withdrawn" }
    ]);
