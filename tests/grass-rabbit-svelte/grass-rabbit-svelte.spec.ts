...
test("connects the Svelte controller to the authored rabbit interaction", async ({
  page
}) => {
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: /AVAL, at home in Svelte\./u })
  ).toBeVisible();
  ...
  await expect(player).toHaveAttribute("aria-describedby", "rabbit-instructions");
  await expect(player).toHaveAttribute("bindings", "auto");
  await expect(player.locator(":scope > source")).toHaveCount(4);
  expect(
    await player.locator(":scope > source").evaluateAll((nodes) =>
      nodes.map((node) => ({
  ...
  if (initialPlayer === null) {
    throw new Error("Expected the Grass Rabbit player host");
  }
  ...
  expect(
    await player.evaluate(
      (element, initialElement) => element === initialElement,
      initialPlayer
    )
  ).toBe(true);
  await expect(player).toHaveCount(1);
...
test("presents reduced motion as a static experience", async ({ page }) => {
  ...
  await expect(
    page.getByText("This animation is inactive under the current runtime policy.")
  ).toBeVisible();
